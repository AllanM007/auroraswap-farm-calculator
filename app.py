...
async def get_farm_apr():
    ...
    poolInfo = BRL_CHEF.functions.poolInfo(1).call()
    ...
    usdTokenRewardsPerWeek = poolRewardsPerWeek * BRLTokenPrice
    ...
    weeklyAPR = (usdTokenRewardsPerWeek / stakedTokens) * 100
    dailyAPR = str(round((weeklyAPR / 7), 2)) + "%"
    weekAPR = str(round(weeklyAPR, 2)) + "%"
    annualAPR = str(round((weeklyAPR * 52), 2)) + "%"


    poolData = {
        "poolTokenAddress": poolInfo[0],
        "poolTokenPrice": "$" + str(round(newtokenPrice, 2)),
        "poolPair": getTokenPair,
        "stakedTokens": str(round(formattedStakingAddressBalance, 2)) + " BRL",
        "stakedTokensUsd": "$" + str(round(usdTokenRewardsPerWeek, 2)),
        "poolRewardsPerWeek": str(round(poolRewardsPerWeek, 2)) + " BRL",
        "poolRewardsPerWeekUsd": "$" + str(round(stakedTokens, 2)),
        "Daily APR": dailyAPR,
        "Weekly APR": weekAPR,
        "Annual APR": annualAPR
    }
